...
trait SyntaxTrait
{
    ...
    public function escapeKey(string $key): string
    {
        if (preg_match('(^([\w(]+)(' . str_replace('_', '.*',
            preg_quote($this->_grammar()->escapeId('_'))) . ')([ \w)]+)$)', $key, $match)) {
            //! columns looking like functions
            return $match[1] . $this->_grammar()->escapeId($this->_grammar()->unescapeId($match[2])) .
    ...
    public function removeDefiner(string $query): string
    {
        return preg_replace('~^([A-Z =]+) DEFINER=`' .
            preg_replace('~@(.*)~', '`@`(%|\1)', $this->_driver()->user()) .
    ...
    private function nextQueryPos(QueryDto $queryDto)
    {
        ...
        $parse = $this->_driver()->sqlStatementRegex();

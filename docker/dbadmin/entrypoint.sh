...
if [ "$1" = "unitd" ] || [ "$1" = "unitd-debug" ]; then
    if /usr/bin/find "/var/lib/unit/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
        ...
        /usr/local/sbin/$1 --control unix:/var/run/control.unit.sock
        ...
        kill -TERM $(/bin/cat /usr/local/var/run/unit/unit.pid)
        ...
        if [ -S /var/run/control.unit.sock ]; then
            kill -KILL $(/bin/cat /usr/local/var/run/unit/unit.pid)
